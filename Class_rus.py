...
class TestPreprocess:
    ...
    def process(self, text, max_seq_length=128, batch_size=16):
        input_ids_full = []
        attention_masks = []
        nopad = []
...
class ProcessOutput:
    ...
    def check_in_dictionary(self, existing_words, word, place, tok_error_type, word_correct, dicty, correct_text,
        ...
        if word_correct.lower() in existing_words:
            # word_correct_with_braces = re.compile(r'{'+word_correct+r'}')
            correct_text = correct_text.replace(word, '{{'+word_correct+r'}}')
            dicty[place] = [word + "->" + word_correct, str(error_prob), tok_error_type,
                            len(word_correct)]
        elif default_value == 'Incorrect':
            dicty[place] = [word, str(1 - hold_prob), "Ошибка, но исправление невозможно", len(word)]
